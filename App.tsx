...
const SystemSequence = ({ type, onComplete }: { type: 'BOOT' | 'SHUTDOWN' | 'RESTART', onComplete: () => void }) => {
    ...
    useEffect(() => {
        const bootLogs = [
            ...
            "Starting Yashika OS Kernel v6.9.0...",
            "Loading modules...",
        ...
        const targetLogs = type === 'BOOT' ? bootLogs : shutdownLogs;
        let delay = 0;
    ...
    return (
        <div className="fixed inset-0 bg-black z-[10000] flex flex-col items-center justify-center text-white font-mono overflow-hidden">
            ...
            <div className="text-2xl font-bold tracking-widest mb-8 animate-pulse text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600">
                {type === 'BOOT' ? 'INITIALIZING SYSTEM' : (type === 'RESTART' ? 'REBOOTING SYSTEM' : 'SYSTEM SHUTDOWN')}
            </div>

            <div className="w-full max-w-2xl h-48 overflow-hidden relative">
                <div className="absolute bottom-0 left-0 w-full flex flex-col justify-end">
                    {logs.map((log, i) => (
                        <div key={i} className="text-xs text-green-500 mb-1 font-mono">
