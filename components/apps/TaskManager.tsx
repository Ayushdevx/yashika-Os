...
const SYSTEM_PROCESSES: Process[] = [
  { pid: 1, name: 'systemd', user: 'root', cpu: 0.1, mem: 0.5, disk: 0.1, priority: 'High', status: 'Sleeping' },
  { pid: 2, name: 'kthreadd', user: 'root', cpu: 0.0, mem: 0.0, disk: 0.0, priority: 'High', status: 'Sleeping' },
  { pid: 1042, name: 'gnome-shell', user: 'yashika', cpu: 2.5, mem: 5.2, disk: 1.2, priority: 'Normal', status: 'Running' },
  { pid: 1840, name: 'dbus-daemon', user: 'messagebus', cpu: 0.0, mem: 0.2, disk: 0.0, priority: 'Normal', status: 'Sleeping' },
  { pid: 2109, name: 'Xorg', user: 'root', cpu: 1.2, mem: 3.4, disk: 0.5, priority: 'High', status: 'Running' },
];
...
const TaskManager: React.FC<AppProps> = () => {
  ...
  useEffect(() => {
    const updateProcesses = () => {
      ...
      const allProcesses = [...systemProcs, ...windowProcesses.map(p => ({ ...p, disk: Math.random() * 2, priority: 'Normal' as const }))];
      ...
      <div className="flex-1 overflow-auto bg-[#1e1e1e]">
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 bg-[#252526] text-gray-400 font-bold shadow-sm z-10">
            <tr>
              <th className="p-2 border-b border-gray-700 w-16 cursor-pointer hover:bg-white/5">PID</th>
              <th className="p-2 border-b border-gray-700 cursor-pointer hover:bg-white/5">Name</th>
              <th className="p-2 border-b border-gray-700 w-24 cursor-pointer hover:bg-white/5">User</th>
              <th className="p-2 border-b border-gray-700 w-24 cursor-pointer hover:bg-white/5">Priority</th>
              <th className="p-2 border-b border-gray-700 w-24 cursor-pointer hover:bg-white/5">Status</th>
              <th className="p-2 border-b border-gray-700 w-20 text-right cursor-pointer hover:bg-white/5">CPU%</th>
              <th className="p-2 border-b border-gray-700 w-20 text-right cursor-pointer hover:bg-white/5">MEM%</th>
              <th className="p-2 border-b border-gray-700 w-20 text-right cursor-pointer hover:bg-white/5">Disk (MB/s)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-800">
            {processes.map((proc) => (
              ...
              >
                ...
                <td className="p-2 text-gray-400">{proc.priority}</td>
                ...
                <td className="p-2 text-right font-mono">{proc.disk.toFixed(1)}</td>
              </tr>
            ))}
          </tbody>
        </table>
